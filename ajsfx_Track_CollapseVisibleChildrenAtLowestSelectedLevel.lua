function ajsfx_Track_CollapseVisibleChildrenAtLowestSelectedLevel()
  ...
  -- Find all tracks with the lowest nesting level and collapse them,
  ...
  for i = 0, track_count - 1 do
    ...
    -- If the current track has the lowest nesting level and its parent is not collapsed, collapse it
    if current_level == lowest_level then
      local parent_collapsed = false
      if parent_track then
        parent_collapsed = reaper.GetMediaTrackInfo_Value(parent_track, "I_FOLDERCOMPACT") > 0
...
ajsfx_Track_CollapseVisibleChildrenAtLowestSelectedLevel()