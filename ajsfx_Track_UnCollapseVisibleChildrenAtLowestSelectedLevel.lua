function ajsfx_Track_UncollapseVisibleChildrenAtLowestSelectedLevel()
  ...
  -- Find the lowest nesting level among VISIBLE selected tracks
  local lowest_level = 1000 -- Initialize with a high value
  for i = 0, selected_tracks - 1 do
    ...
    if current_level < lowest_level then
  ...
  if lowest_level == 1000 then return end

  -- Find all tracks with the lowest nesting level and uncollapse them,
  ...
  for i = 0, track_count - 1 do
    ...
    -- If the current track has the lowest nesting level and its parent is not collapsed, uncollapse it
    if current_level == lowest_level then
      local parent_collapsed = false
      if parent_track then
        parent_collapsed = reaper.GetMediaTrackInfo_Value(parent_track, "I_FOLDERCOMPACT") > 0
...
ajsfx_Track_UncollapseVisibleChildrenAtLowestSelectedLevel()